...
cmux send-key-panel --panel surface:3 enter
...
>
> ```bash
> cmux send --surface <sp> "<message>"
> cmux send-key --surface <sp> enter
> ```
>
> 이 두 줄을 항상 쌍으로 실행한다. 단일 셸 명령을 보낼 때도 `\n` 대신
> `send-key enter`를 권장(혼동 방지 + TUI/셸 공통 처리).
